...
P1(로그인·회원가입·상품목록/검색·상품상세·내상품·상품등록/수정·마이페이지) 완료.
**현재 웹 Phase 2 진행 중**(찜·문의·견적·미팅·거래·리뷰·알림·관리자) — 작업 순서는 `WEB-PHASE2-TASKS.md`를 따른다.

## 작업 범위(Scope)

- **`apps/web` 디렉토리만** 다룬다. 백엔드(`apps/api`)는 수정하지 않는다.
- 화면은 **`screen-spec.md`의 P1·P2 화면**(COM/BUY/SEL/ADM)까지. 묶음 순서는 `WEB-PHASE2-TASKS.md`.
- API 요청/응답은 **`api-spec.md` 계약을 그대로** 따른다(필드명·구조 임의 변경 금지).
- 범위 밖(P3: 결제·추천·분쟁 등) 화면은 만들지 않는다.
...
- 백엔드(`apps/api`) 코드 수정, 범위 밖(P3) 화면 생성.
